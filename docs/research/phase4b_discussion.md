...
This effect is observed at the branch level: 18 of 20 contaminated branches display positive representational leakage ($L_{\mathrm{repr}} > 0$), and 10 of 20 achieve nominal branch-level significance ($p < 0.05$) under non-parametric sign-flip permutation tests. Furthermore, event-level analysis reveals that prediction error reductions are distributed across 71.9% of out-of-sample FOMC meetings, demonstrating that the signal reflects generalized representation alignment rather than an artifact of a single macroeconomic event.
...
A naive intuition might assume that temporal leakage should obey a monotonic law: higher contamination dose ($D$) should strictly produce greater representational leakage ($L_{\mathrm{repr}}$). The empirical evidence directly refutes this intuition. Across the five optimization seeds, mean $L_{\mathrm{repr}}$ increases from $+0.00368$ at $D = 0.25$ to an aggregate peak of $+0.00507$ at $D = 0.75$, but declines to $+0.00291$ at $D = 1.00$ ([Figure 1](file:///e:/MANTRA/docs/research/figures/phase4b/figure1_l_repr_dose_response.png)). In Seed 123, point estimates actually fall below clean baseline performance at $D = 0.50$ and $D = 1.00$.
...
Phase 4B demonstrates that **seed-dependent optimization variance is a first-order effect** in temporal leakage studies ([Figure 2](file:///e:/MANTRA/docs/research/figures/phase4b/figure2_branch_significance_map.png)). Although all branches within a seed shared identical initial weights and masking schedules, results diverged markedly across the five optimization seeds:
...
Because compute, text genre, and vocabulary domain were strictly equalized, generic domain adaptation cannot account for the difference in future-rate decodability between clean and contaminated twins. However, this design cannot rule out that subtle distribution shifts in post-2020 central-bank vocabulary (e.g., changes in rhetorical emphasis) coincidentally enhanced probe linearity. We discuss this boundary in [Section 6 (Limitations)](file:///e:/MANTRA/docs/research/phase4b_limitations.md).
...
- **Phase 4B Confirmatory Study**: Motivated by the power calculations of Phase 3, Phase 4B scaled the evaluation to 40 total events ($N_{\mathrm{OOS}} = 32$ out-of-sample independent events), 5 random seeds, 5 contamination doses, and 25 total empirical branches on CUDA hardware ($6,400,000$ total tokens processed).
